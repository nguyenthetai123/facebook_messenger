...
if ( ! class_exists( 'WPE_Core_Options' ) ) {
	...
	class WPE_Core_Options {
		...
		public function options_fields(){
			$_GET = Wpe_Helper::sanitize_data( $_GET );
			...
			foreach ($fields as $key => $field) {
				...
				if(in_array($field['type'], $allow)) {
					...
					if(isset($_REQUEST['ID']) && !empty($_REQUEST['ID'])) {
						$_REQUEST = Wpe_Helper::sanitize_data( $_REQUEST );
		...
		public function save_options(){
			...
			$_POST = Wpe_Helper::sanitize_data($_POST);
			...
			foreach ($_POST as $key => $value) {
				if(!Wpe_Helper::update_option($key, $value)) {
					$result = array(
						'status' => 'error',
		...
		public function save_post(){
			...
			foreach ($_POST as $key => $value) {
				if(!Wpe_Helper::update_meta($ID, $key, $value)) {
					$result = array(
						'status' => 'error',
						'title' => esc_html__('Error','wpelite'),
